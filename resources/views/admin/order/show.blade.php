    ...
    <div class="right_col" role="main" style="min-height: 2000px">
        <div class="row">
            <div class="col-12">
                <div class="status">
                    @if (session('status'))
                        <h6 style="font-size:16px !important" class="alert alert-success">{{ session('status') }}</h6>
                    @endif
                </div>
                <div class="card">
                    <div class="card-header">
                        <h4 class="d-flex justify-content-between"> Chi tiết đơn hàng
                            <a href="{{ route('getListOrder') }}" class="btn btn-primary float-end">Trở lại trang trước</a>
                        </h4>
                    </div>
                    <div class="card-body" style="font-size: 20px">
                        <form action="{{ URL::to('/admin/orders/' . $order->id . '/confirm') }}" method="POST"
                            ...
                            <div class="form-group">
                                ...
                                <input type="number" name="price" id="price" class="form-control"
                                    value="{{ $order->peopleNumber }}" disabled>
                            </div>

                            <div class="form-group">
                                <label for="image">Ngày tổ chức:</label>
                                <input type="text" name="image" id="image" class="form-control"
                                    value="{{ $order->organizationDate }}" disabled>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                                ...
                                                @foreach ($order_foods as $order_food)
                                                    <tr class="even pointer">
                                                        ...
                                                        <td class="align-items-center ">
                                                            {{ number_format($order_food->food->price, 0) }}</td>
                                                        <td class="align-items-center ">
                                                            <img src="{{ asset($order_food->food->image) }}" width="70px"
                                                                height="70px" alt="">
                                                        </td>
                                                    </tr>
                                                @endforeach
                                ...
                                {{-- <input type="text" name="image" id="image" class="form-control" value="{{$order->payment->name}}"
                disabled> --}}
                            </div>
                    </div>
